...
for(i in 1:length(gps.behav.data.list)) print(paste(i, gps.behav.data.list[[i]]$birdID[1])) # to see the i of each individual
...
for (i in 1:length(gps.behav.data.list)) {
  ...
  df$week <- week(df$date_time_CEST)
  df$freq <- 1
  ...
  df$sunset=sunriset(coordinates(df), df$date_time_CEST, POSIXct.out=TRUE, direction="sunset")[,2]
  df$sunrise=sunriset(coordinates(df), df$date_time_CEST, POSIXct.out=TRUE, direction="sunrise")[,2]
  df$daynight = ifelse(df$date_time_CEST<df$sunrise|df$date_time_CEST>df$sunset,"night","day")
  df$dusk=crepuscule(coordinates(df), df$date_time_CEST, solarDep = 6,POSIXct.out=TRUE, direction="dusk")[,2]
  df$dawn=crepuscule(coordinates(df), df$date_time_CEST, solarDep = 6, POSIXct.out=TRUE, direction="dawn")[,2]
  df$twilight_day_night = ifelse(df$date_time_CEST<df$dawn|df$date_time_CEST>df$dusk,"night","day")
  # add habitat
  habitats.to.add <- over(df, schier_new84_sel) 
  df <- data.frame(df, habitats.to.add[,c('habitat','habitat_type','habitat_salinity','land_water')]) # this function does not yet remove GPS points outside the habitat map, but gives NA's for the habitat columns in those cases  
  ...
  test=df.list.breeding.year[[j]][,c('date_time_CEST','breeding.phase','attempt')]
  ...
  if (length(df.list.breeding.year)>0) {
    ...
    names(gps.breeding.data.behav.list)[i]<-birdID
  }
}
...
# add hatch day per corresponding breeding attempt: 
...
# ONDERSTAANDE IS DENK IK NIET MEER AAN DE ORDE
