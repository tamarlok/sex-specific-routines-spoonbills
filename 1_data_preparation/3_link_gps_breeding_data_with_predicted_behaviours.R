...
gps.breeding.data.behav.known <- gps.breeding.data.behav[gps.breeding.data.behav$behaviour!="unknown" & gps.breeding.data.behav$habitat!="unknown",]
...
dim(duration_yday_birdID_year_all) # 4787 birddays
names(duration_yday_birdID_year_all)[8] = 'dur_per_yday'
...
#gps.breeding.data.behav.sel <- gps.breeding.data.behav.known
...
gps.breeding.data.behav.sel$behaviour2[gps.breeding.data.behav.sel$distance.to.nest<5] = 'at_nest'
...
#gps.breeding.data.behav.sel$behaviour2[gps.breeding.data.behav.sel$nest1==1|gps.breeding.data.behav.sel$nest2==1] = 'at_nest'

gps.breeding.data.behav.sel$behaviour2.nr <- NA
gps.breeding.data.behav.sel$behaviour2.nr[gps.breeding.data.behav.sel$behaviour2=='foraging_marine'] = 9
gps.breeding.data.behav.sel$behaviour2.nr[gps.breeding.data.behav.sel$behaviour2=='foraging_brackish'] = 8
gps.breeding.data.behav.sel$behaviour2.nr[gps.breeding.data.behav.sel$behaviour2=='foraging_freshwater'] = 7
gps.breeding.data.behav.sel$behaviour2.nr[gps.breeding.data.behav.sel$behaviour2=='flying'] = 6
gps.breeding.data.behav.sel$behaviour2.nr[gps.breeding.data.behav.sel$behaviour2=='walking'] = 5
gps.breeding.data.behav.sel$behaviour2.nr[gps.breeding.data.behav.sel$behaviour2=='resting_wadden'] = 4
gps.breeding.data.behav.sel$behaviour2.nr[gps.breeding.data.behav.sel$behaviour2=='resting_mainland'] = 3
gps.breeding.data.behav.sel$behaviour2.nr[gps.breeding.data.behav.sel$behaviour2=='resting_schier'] = 2
gps.breeding.data.behav.sel$behaviour2.nr[gps.breeding.data.behav.sel$behaviour2=='at_nest'] = 1
...
gps.breeding.data.behav.sel <- gps.breeding.data.behav.sel[gps.breeding.data.behav.sel$behaviour!="unknown"&gps.breeding.data.behav.sel$behaviour2!="foraging_NA"&gps.breeding.data.behav.sel$behaviour2!="resting_NA"&gps.breeding.data.behav.sel$behaviour2!="resting_rest",] # this could affected the calculated total durations, so we should eiter make that selection after this line, or recalculate the total durations. 
# similar, we should also use some criteria to define foraging_land better
gps.breeding.data.behav.sel$behaviour2[gps.breeding.data.behav.sel$behaviour2=="foraging_land"&gps.breeding.data.behav.sel$habitat_type=="schier"] <- "foraging_brackish" # this is not necessarily true!
gps.breeding.data.behav.sel$behaviour2[gps.breeding.data.behav.sel$behaviour2=="foraging_land"&gps.breeding.data.behav.sel$habitat_type=="mainland"] <- "foraging_freshwater"
# remove the 5 points with habitat_type=rest
gps.breeding.data.behav.sel <- gps.breeding.data.behav.sel[gps.breeding.data.behav.sel$habitat_type!="rest",]



# check again where to put the criterium of removing data
duration_yday_birdID_year_all <- aggregate(duration_behav/60~year+birdID+sex+yday_CEST+week+breeding.phase+breeding.phase.nr, gps.breeding.data.behav.sel[gps.breeding.data.behav.sel$breeding.phase.nr>2,], sum) 
dim(duration_yday_birdID_year_all) # 4757 birddays
names(duration_yday_birdID_year_all)[8] = 'dur_per_yday'
distr.duration <- table(round(duration_yday_birdID_year_all$dur_per_yday,1), duration_yday_birdID_year_all$birdID)
distr.duration[(dim(distr.duration)[1]-20):dim(distr.duration)[1],] # now it is back to 23.8 hours. 
...
dim(duration_yday_birdID_year) ## 3982 birddays left (4757-3982 = 775 birddays removed)
...
ydays.year.bird.phase.behav.known <- unique(gps.breeding.data.behav.known[,c('year','birdID','sex','breeding.phase.nr','attempt','yday_CEST','week','day_rel_hatching','week_rel_hatching')])
duration_yday_birdID_year = merge(ydays.year.bird.phase.behav.known, duration_yday_birdID_year[c('year','birdID','yday_CEST','breeding.phase' ,'dur_per_yday')], by=c('year','birdID','yday_CEST')) # to add the columns rel_day_hatching and rel_week_hatching to the duration_yday_birdID_year dataframe.

gps.breeding.data.behav.sel = merge(gps.breeding.data.behav.known, duration_yday_birdID_year[,c('year','birdID','yday_CEST')]) # remove the days with durations<=23.1 in the gps.breeding.data.behav.known file, as well as the days in the pre-breeding or (very broad) breeding phase. 
...
dim(gps.breeding.data.behav.known) # 920079 rows
dim(gps.breeding.data.behav.sel) # 713951 rows 
...
keep(ColHabitats, gps.breeding.data.behav.sel, ydays.year.bird.phase.all, ydays.year.bird.phase.behav.known, ydays.year.bird.phase.sel, duration_yday_birdID_year, bird.data, breeding.data, sure=T)
